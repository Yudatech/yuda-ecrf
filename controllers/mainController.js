...
async function calculateKpis(user) {
  ...
  kpis.discontinuationRate = kpis.finished === 0 ? 0 : ((getCaseListByStatus(caseList, 'quit').length * 100 / (kpis.finished + getCaseListByStatus(caseList, 'quit').length))).toFixed(2) + '%';
